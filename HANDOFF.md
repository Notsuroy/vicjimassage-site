...
| 2026-05-28 | **Vicji batch 8 feedback (5 areas, 8 files)**: /servicos contraindicação block `pra gente decidir juntos` → `para decidirmos juntos` (now matches the FAQ wording from batch 6). /servicos + /en/services international payment line simplified: `aceito reais, dólar ou euro em dinheiro, além de cartão internacional` → `dinheiro ou cartão internacional` (less verbose; currencies still listed elsewhere). /blog + /en/blog index taglines drop redundant `sobre`/`about`: `sobre presença, sobre cuidado` → `sobre presença e cuidado`. /agendar + /en/book 3-step blocks tightened: Step 1 `Você manda mensagem` → `Você envia uma mensagem`; Step 2 `te mando o endereço do estúdio` → `te informo o meu endereço`; Step 3 `pra sessão` → `para a sessão` (+EN mirrors). Blog post `antes-da-primeira-sessao.md` (PT + EN equivalent): deleted `Como ir pra uma aula de yoga.` / `Like going to a yoga class.` (redundant), lowercased `"Top"` → `top` (no quotes). Commit `9a58dfb`. Follow-up `2deb968`: deleted `top` / `sports top` entirely from clothing list (Vicji's original intent). |
| 2026-05-28 | **Vicji batch 9 feedback (~30 edits across 3 PT blog posts, PT-only — EN mirrors deferred until Vicji reviews EN)**. `antes-da-primeira-sessao.md`: Section 3 ritual rewrites (intake question expansion, `respirações profundas e completas`, `realizo um minuto de meditação`, `parte essencial`), Section 4 title `Como te comunicar` → `Como se comunicar`, bullet tightening (`reduz a pressão`, `me diga no momento`, `eu ajusto na hora`), Section 5 `Vontade de dormir` adds `e bom`, `Suspiros profundos` rewrite, deleted `Sentir mais um lado do corpo` bullet, closing CTA `Já marca a sua primeira sessão`. `massagem-tailandesa-para-dor-lombar.md`: frontmatter `gente acumula` → `pessoas acumulam`, `a partir de uma visão integrada`, `Para a dor lombar`, step 2 `o posterior da coxa`, step 3 expansion (`e outras, mas eu sustento`), dropped price from Liberação section, fixed `pro perfil`/`com acompanhamento`/`substituo médico`/`procura uma avaliação`, `Quem se beneficia` (`muito tempo sentado, má postura`, `não se recuperam`), `O trabalho mais profundo` rewrite (`a lombar ou o quadril começam a soltar`, `o porquê`, `também pede passagem`, `Não é sempre que acontece... é parte do trabalho, e é lindo. A cura também vem daí.`), final CTA tightened (`Experimenta a Liberação`, drops price, `no corpo inteiro`, drops address line). `o-que-e-massagem-tailandesa.md`: `Nuad Boran` translation `Massagem antiga` (was `trabalho ancestral`), dropped `não é técnica de spa...` tail, `postura e tradição por trás delas`, Três níveis intro feminine agreement fix (`Ela nasceu integrada, e não fragmentada em partes`, `executada`), futon section adds `com a gravidade e a biomecânica a favor`, `roupas confortáveis` plural, `aquela sensação de` (was `pegada`), `soltando os bloqueios`, `Eu conduzo por você` (was `Eu faço`), benefits list fixes (`no quadril e na coluna`, `lombar, cervical e quadris`, `nas noites seguintes`), `o porquê`/`explicar muito`, Quem se beneficia (`acumulam tensões em ombros, pescoço, quadris e costas`, truncated estresse bullet), `Massagem tailandesa tradicional` opener for comparison, `Sessões mais longas`, dropped `profundamente`, two-sessions list (`muitos anos com tensão acumulada`, `Mais curta mas ainda sim profunda`), `Após, volto` comma, deleted `Pra muita gente, a primeira sessão é só o começo...` paragraph, Vale-a-pena closing rewrite (`Te garanto que, ao menos, você não irá se arrepender` + `será sempre um prazer recebê-lo de volta`), final CTA tightened (`Já agenda a sua sessão`, drops address line). Commit `f72baf9`. **EN mirror tracked as task #34**. |
| 2026-05-29 | **Vicji batch 10 feedback (/sobre PT + MiniAbout, EN mirrors deferred for /sobre)**: H1 `pra casa pra compartilhar` → `para casa, para compartilhar`; para 3 `pra Tailândia` → `para a Tailândia`; para 4 `De lá fui pra Índia` → `De lá, fui para a Índia` + `os bloqueios e o que faz` → `os bloqueios, e o que faz` + `sobre o corpo, a energia e a presença` → `sobre o corpo, energia, e presença` (drops articles, Oxford comma); para 5 `Voltei pro Brasil porque` → `Voltei para o Brasil, porque`; pricing philosophy: `descanso entre atendimentos` → `descanso entre os atendimentos`, `por semana porque cada um pede entrega completa, e eu não quero entregar menos` → `por semana, porque cada um pede uma entrega completa, e eu me recuso a entregar menos` (**`me recuso a` is much stronger than `não quero`** — adds boundary-setting confidence), `massagem de spa porque` → `massagem de spa, porque`. **MiniAbout component (PT + EN both)**: deleted third body[] paragraph `Atendo em Atibaia, no Espaço Clô. Em breve em Itacaré.` / `I see clients in Atibaia, at Espaço Clô. Soon in Itacaré.` — same consolidation as batches 8 + 9, drop location callouts from per-page content since Footer + LocalBusiness schema carry them. Commit `1627d56`. |
| 2026-05-29 | **Vicji batch 11 + 12 feedback (FINAL PT EDITORIAL REVIEW PASS)**. Vicji's closing message: "That's it man!!!! Guess it's done!!!!" — site copy is locked from Vicji's side. **FAQ (PT + EN both)**: `Se é sua primeira vez` → `Se é a sua primeira vez` (article); drops `se possível` from vestir; adds comma `sessão, e faço`; `algo é demais` → `algo está demais` (state, not category); question `pra` → `para`; sueca answer ending `Não é técnica vazia, é prática milenar.` → `É prática milenar de cura.` (drops negation, adds healing dimension); `mais profunda que` → `mais profunda do que`; `ja confia` → `já conhece e confia` (Vicji's pair-expansion pattern). **/blog index**: `pra quem quer entender` → `para quem quer entender`. **antes-da-primeira-sessao.md (PT)**: frontmatter `pra primeira vez` → `para a primeira vez`; vestir bullet `Camiseta de manga curta ou regata` → `Camiseta, top, ou regata` (Vicji wants `top` back, comma-separated this time); drop `duas horas antes` from iogurte bullet; `Hidrata-te` → `Hidrate-se` (você imperative for site-wide consistency); `modo descanso` → `modo de descanso`. **o-que-e-massagem-tailandesa.md (PT)**: `ou pra quem ouviu` → `ou para quem ouviu`; Energético `O corpo descansa do que estava represado` → `O corpo libera o que estava represado` (active over passive — propagating the ProblemPromise batch 6 fix to the blog); Espiritual comma-join + `É prática integrativa milenar`; futon section rewrite `(pés, joelhos...) e pra você receber em diferentes posições, com a gravidade e a biomecânica a favor` → `(pés, joelhos...) com o auxílio da gravidade e da biomecânica` (drops redundant clause); `esforço pra chegar` → `esforço para chegar`; benefits list adds `ombros` to two bullets; `vai além do físico` → `vai muito além do físico`; Liberação de Quadril bullet `Mais curta mas ainda sim profunda` → `Mais curta, mas ainda sim profunda` (comma); `vale a pena pra sempre` → `vale a pena para sempre`. EN blog mirrors still tracked under task #34 — defer until Vicji reviews English versions. Commit `742e6c6`. |
| 2026-09-09 | **Services split into 3 named sessions with dedicated pages (Suroy-directed).** Thai Tradicional's two duration variants became two separate services: **Thai Massage Tradicional (2h, R$360)** and **Thai Massage Imersiva (3h, R$520)**; Liberação de Quadril (1h30, R$290) unchanged. Naming rationale: Suroy proposed "full" (2h) / "complete immersive" (3h), but *completa/complete* and *full* read as synonyms in both languages and "sessão completa" is already the generic descriptor for both Thai sessions site-wide, so the differentiator is now Tradicional vs **Imersiva** (Vicji's own word, already in the old `3 horas (imersiva)` label). **Prices and durations untouched, still Vicji's own numbers.** New detail pages at `/servicos/<slug>` + `/en/services/<slug>`: `massagem-tailandesa-tradicional` / `traditional-thai-massage`, `massagem-tailandesa-imersiva` / `immersive-thai-massage`, `liberacao-de-quadril` / `hip-release`. `/servicos` + `/en/services` stay as hubs. Each detail page: hero with price + duration + city, 4-paragraph intro, "Para quem é" block (the real 2h-vs-3h differentiator, written for different intent so the two Thai pages don't cannibalize: 2h = first/standard session, 3h = deep dive + Itacaré holiday audience), highlights, included, 5-moment session structure, preparation + contraindications, payment/cancellation, 5 page-specific FAQs, cross-links to the other two sessions. ~840-860 words each. Schema per page: Service + Offer (price, `durationISO`) + FAQPage + BreadcrumbList; hub OfferCatalog offers now carry `url` + `durationISO` (drops the old `PT${...}H` regex hack). services.json restructured: each service has one `options[]` entry plus new `slug{pt,en}`, `metaTitle`, `metaDescription`, `intro[]`, `forWhom[]`, `faqs[]`, and `medicalNote` (hip page only, red-flag screen per the §6b.3 pattern from the dor-lombar post). **New `src/data/routes.ts`** is now the single source of truth for the PT↔EN slug map, which was duplicated in `Header.astro` + `BaseLayout.astro` and would have silently dropped every new page's hreflang to the homepage; both now import it, and Header nav highlights a parent when on a nested path. `site.json` brand.fullDescription (canonical copy, also used for GBP + LocalBusiness schema) updated to name all three sessions. ServicesGrid: 3-up grid, card titles + "Ver detalhes desta sessão" link through to detail pages, `recommended` badge reworded `Sessão completa` → `Mais procurada` (it no longer distinguishes the card). Sitemap generates the 6 new URLs from services.json. Verified: `npm run audit:voice` fully clean (0 HARD, 0 WARN), 25 pages built, all 6 hreflang pairs cross-link correctly (not homepage fallbacks), all internal links resolve, Service+FAQPage+BreadcrumbList present on all 6, FAQ schema matches visible text, meta descriptions ≤160. **Open with Vicji: confirm the "Imersiva" name before it gets indexed** — renaming later means 301s. |
